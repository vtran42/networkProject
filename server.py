...
def clientThread(conn):
    ...
    print("receiving message: ", rcv_msg)
    print type(rcv_msg)
        ...
        while True:
            ...
            if option == str(1):
                ...
                for i in range(len(userpass)):
                    print rcv_msg[0] == userpass[i][0]
                    if rcv_msg[0] == userpass[i][0]:
                        userpass[i] = rcv_msg

                print 'successful change the password'
                print userpass
            else:
                try:
...
def receiveClients(s):
    ...
    while 1:
        ...
        clients.append(conn)
        # Tips: start new thread takes 1st argument as a function name to be run, second is the tuple of arguments to
        # the function.
